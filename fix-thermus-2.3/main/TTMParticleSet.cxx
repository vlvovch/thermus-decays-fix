...
//__________________________________________________________________________
TTMParticleSet::TTMParticleSet(char* file, Bool_t CB)
{
...
void TTMParticleSet::GenerateBRatios(TTMParticle* parent)
{
  ...
  if (parent->GetDecayChainProcessed()) return;  //FIX (V. Vovchenko): Added to fix decay contributions
  ...
  if (!parent->GetStable())    //Parent unstable
      ...
      do {
        ...
        while ((p_decay = (TTMDecay*) p_next())) {
          ...
          if(daughter){			//if daughter is in the set
            //if (daughter->GetStable())	//if daughter is stable, FIX(V. Vovchenko): commented out
              {
              ...
              } //else              // if daughter is unstable, FIX(V. Vovchenko): commented out
              if (!daughter->GetStable()) // FIX(V. Vovchenko)
                {
                  if (!daughter->GetDecayChainProcessed()) GenerateBRatios(daughter); // FIX(V. Vovchenko)
                  ...
                  while ((d_decay = (TTMDecay*) d_next())) {
                    ...
                    //if (grandaughter->GetStable())  FIX(V. Vovchenko): commented out
                      //if grandaughter is stable FIX(V. Vovchenko): commented out
                      {
                        TIter t_next(temp_decays);
                        TTMDecay* t_decay;
                        Bool_t found = 0;
                      ...
                      } // FIX(V. Vovchenko): else statement commented out
                      // else
    ...
    parent->SetDecayChainProcessed(true);  //FIX(V. Vovchenko)
}
